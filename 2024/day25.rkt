...
(define (fmt fn)
  (for/list ((thing (string-split (file->string fn) "\r\n\r\n")))
    ...
    (define type
      (cond ((and (regexp-match hashpat (first lines))
                  (regexp-match dotpat (last lines)))
             'lock)
            ((and (regexp-match dotpat (first lines))
                  (regexp-match hashpat (last lines)))
             'key)
            (else (raise 'badinput))))
    (cons type
          (for/hash ((line lines)
...
(define (pinheight thing)
  ...
  (for/list ((charn (add1 maxchar)))
    (sub1 (for/sum ((linen (add1 maxline))
                    #:do ((define value (hash-ref (cdr thing) (cons linen charn))))
                    #:when (equal? value "#"))
            1))))
...
(define (day25 input)
  ...
  (for*/sum ((key (map pinheight keys))
             (lock (map pinheight locks))
             #:when (andmap identity
                            (for/list ((k key) (l lock))
                              (< (+ k l) max))))
    1))
