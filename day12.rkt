...
(define (region input r key value)
  ...
  (for/list ((t (list up right down left))
    ...
    (cond ((hash-has-key? r next-key) empty)
...
(define (hash-ref-safe ht key)
  (if (hash-has-key? ht key) (hash-ref ht key) empty))

(define (perimeter input r)
  (for*/sum ((key (hash-keys r))
             (t (list up right down left)))
    (if (equal? (hash-ref r key) (hash-ref-safe input (go t key))) 0 1)))
    
(define (part1 input)
  (for/sum ((r (regions input)))
    (* (hash-count r) (perimeter input r))))
...
(define (turns input region value)
  (for/sum ((key (hash-keys region)))
    (count identity
           (for*/list ((h (list left right)) (v (list up down)) (f (list corner? side?)))
             (f input h v key value)))))

(define (part2 input)
  (for/sum ((r (regions input)))
    (* (hash-count r) (turns input r (first (hash-values r))))))
